...
import { ShieldAlert, TrendingDown, DollarSign, Target, CheckCircle2, AlertTriangle, XCircle, ArrowUpRight, ShieldCheck, Activity } from 'lucide-react';
...
export default function MetricsCards({ metricsData, onQuickTest }) {
  if (!metricsData) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-pulse">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 h-28 space-y-3">
  ...
  const costComparisonData = [
    { name: 'Unmitigated Loss', fullName: 'Baseline Unmitigated Loss (No Model)', cost: cost_analysis.baseline_unmitigated_loss_inr, color: '#f43f5e' },
    { name: 'RevShield Cost', fullName: 'RevShield Model Overhead + Friction', cost: cost_analysis.total_model_loss_inr, color: '#38bdf8' },
    { name: 'Net Money Saved', fullName: 'Net Profit Protected for Merchant', cost: cost_analysis.net_financial_savings_inr, color: '#10b981' },
  ];

  return (
    <div className="space-y-6">
      {/* Top Banner Alert */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-950/40 via-slate-900/80 to-indigo-950/40 border border-blue-800/40 p-5 shadow-lg">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-300 max-w-2xl">
              Trained on {dataset?.total_samples?.toLocaleString()} transactions. Calibrated to penalize customer friction (₹{cost_analysis?.cost_per_fp_inr} / FP) and direct chargeback losses (₹{cost_analysis?.loss_per_fn_inr} / FN).
            </p>
          </div>

          <div className="flex items-center space-x-3 shrink-0">
            <div className="bg-slate-950/80 px-4 py-2.5 rounded-xl border border-slate-800 text-left md:text-right">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Precision */}
        <div className="bg-slate-900/60 backdrop-blur-sm rounded-xl p-4 border border-slate-800 hover:border-blue-500/40 transition-all">
        ...
        {/* Recall */}
        <div className="bg-slate-900/60 backdrop-blur-sm rounded-xl p-4 border border-slate-800 hover:border-indigo-500/40 transition-all">
          <div className="flex items-center justify-between">
        ...
        {/* F1 Score */}
        <div className="bg-slate-900/60 backdrop-blur-sm rounded-xl p-4 border border-slate-800 hover:border-cyan-500/40 transition-all">
          <div className="flex items-center justify-between">
        ...
        {/* False-Positive Cost */}
        <div className="bg-slate-900/60 backdrop-blur-sm rounded-xl p-4 border border-slate-800 hover:border-amber-500/40 transition-all">
          <div className="flex items-center justify-between">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-7 bg-slate-900/60 backdrop-blur-sm rounded-2xl p-5 border border-slate-800 flex flex-col justify-between overflow-hidden">
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-semibold text-white">Financial Impact: Baseline vs. RevShield Model</h3>
            </div>
            <p className="text-xs text-slate-400 mb-4">Direct comparison of risk exposure and preserved merchant revenue (INR)</p>

            <div className="h-56 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={costComparisonData} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                      ...
                      if (active && payload && payload.length) {
                        return (
                          <div className="bg-slate-950 border border-slate-700 p-2.5 rounded-xl shadow-xl text-xs space-y-1">
                            <p className="font-semibold text-white">{payload[0].payload.fullName}</p>
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 pt-3 border-t border-slate-800/80 text-center">
            <div>
              ...
              <p className="text-xs sm:text-sm font-bold text-rose-400 font-mono truncate">₹{cost_analysis.baseline_unmitigated_loss_inr.toLocaleString('en-IN')}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-400">With RevShield Incurred</p>
              <p className="text-xs sm:text-sm font-bold text-sky-400 font-mono truncate">₹{cost_analysis.total_model_loss_inr.toLocaleString('en-IN')}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-400">Net Money Saved</p>
              <p className="text-xs sm:text-sm font-bold text-emerald-400 font-mono truncate">+₹{cost_analysis.net_financial_savings_inr.toLocaleString('en-IN')}</p>
            </div>
          </div>
        </div>

        {/* Right Card: Confusion Matrix */}
        <div className="lg:col-span-5 bg-slate-900/60 backdrop-blur-sm rounded-2xl p-5 border border-slate-800 flex flex-col justify-between overflow-hidden">
          <div>
            <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-xs text-slate-400 mb-3">Class distribution on 1,000 unseen test transactions</p>
